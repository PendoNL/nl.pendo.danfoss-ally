...
// Mode → setpoint code mapping. Used to re-emit the setpoint alongside a mode
// change so the Danfoss cloud actually pushes the new state to the TRV.
...
class DanfossAllyApi {
  ...
  async setMode(deviceId, mode) {
    ...
    // Re-emit the setpoint for the target mode in the same batch. A mode-only
    // write is accepted by the cloud but the gateway sometimes does not push
    // the change down to the TRV until a setpoint is written too.
    const setpointCode = MODE_SETPOINT_MAP[mode];
    const cached = this._devices[deviceId];
    if (setpointCode && cached && typeof cached[setpointCode] === 'number') {
      commands.push({ code: setpointCode, value: cached[setpointCode] });
    }
